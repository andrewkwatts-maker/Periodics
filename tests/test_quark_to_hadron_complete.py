...
from utils.simulation_schema import propagate_quark_to_hadron
...
def test_hadron_from_quarks(hadron_data: Dict, quark_data: Dict[str, Dict]) -> Optional[HadronTestResult]:
    """
...
def run_all_tests(data_dir: Path) -> TestSummary:
    ...
    for name, hadron_data in sorted(subatomic_data.items()):
        ...
        result = test_hadron_from_quarks(hadron_data, quark_data)
...
def print_report(summary: TestSummary):
