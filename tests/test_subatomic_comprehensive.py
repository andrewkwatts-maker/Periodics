...
from utils.physics_calculator import SubatomicCalculator, PhysicsConstants
...
def test_particle(particle_data: Dict, is_resonance: bool = False) -> Optional[ParticleTestResult]:
...
def run_default_particle_tests(particles: Dict[str, Dict]) -> Tuple[List[ParticleTestResult], TestStatistics]:
    """Run tests on all default particles and compute statistics."""
    results = []

    for name, data in particles.items():
        result = test_particle(data)
        if result:
...
def test_exotic_hadrons() -> List[ExoticParticleTest]:
...
def main():
    ...
    exotic_results = test_exotic_hadrons()
